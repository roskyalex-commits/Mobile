 ...
 *
 * Nothing persists yet — the confirm handler will POST to /api/v1/agents once
 * auth lands. Everything up to that point is real: the site is fetched and the
 * ICP is genuinely inferred by Claude.
 */
        ...
        <Card className="p-6">
          ...
          <div className="flex justify-between">
            ...
            <Button
              ...
              onClick={() => {
                setLaunching(true);
                // TODO(persistence): POST to /api/v1/agents, then route to the
                // created agent. Until then this lands on the agents list.
                router.push("/app/agents");
              }}
            >
              <Rocket className="h-4 w-4" aria-hidden />
              {launching ? "Launching…" : "Create agent"}
            </Button>
          </div>

          <p className="mt-4 text-xs text-muted">
            Saving needs the database. Create the Supabase project and run{" "}
            <code className="font-mono">npm run db:setup</code> — until then the
            app runs on sample data.
          </p>
        </Card>
      )}
    </div>
